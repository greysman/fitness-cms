...
<div>
    <div class="flex gap-4 items-center pt-2 pb-2">
        <div>
            <x-dynamic-component :component="config('timex.pages.buttons.modal.view.contact','heroicon-o-user')" class="h-5 w-5" />
        </div>
        <div class="flex gap-2">
            @php
            $contact = \App\Models\Crm\Contact::find($contact_id)
            @endphp
            <div class="flex items-center gap-2">
                <div @class([ 'w-7 h-7 rounded-full bg-gray-200 bg-cover bg-center' , 'dark:bg-gray-900'=> config('filament.dark_mode'),
                    ])
                    style="background-image: url('{{ \Filament\Facades\Filament::getUserAvatarUrl($contact) }}')">
                </div>
                <div class="text-sm">
                    {{$contact->fullname}}
                </div>
    ...
    <div class="flex gap-4 pb-2 items-center text-xs">
        ...
        <div @class([ 'bg-' .$color.'-500/10 text-'.$color.'-500'=> $color !== 'secondary',
    ...
    <div class="grid grid-cols-1 divide-y border rounded-md mt-2">
        ...
        <div class="p-2 flex justify-between">
            ...
            <div class="flex gap-2">
                <x-filament-support::icon-button :icon="'heroicon-o-download'" :size="'sm'" wire:click="loadAttachment('{{$attachment}}')" />
            </div>
        </div>
        @endforeach
    </div>
